...
const VARIANT_BATCH_SIZE = 75; // Reduced batch size to avoid URL length issues
...
export default function HomeScreen() {
  ...
  const loadProducts = useCallback(async () => {
    try {
      ...
      // STEP 2: Fetch ALL products at once (no batching for products)
      ...
      // STEP 3: Load variants in BATCHES to avoid URL length issues
      ...
      if (productIds.length > 0) {
        const batchCount = Math.ceil(productIds.length / VARIANT_BATCH_SIZE);
        console.log(`  Will load variants in ${batchCount} batches of ${VARIANT_BATCH_SIZE} products each`);
        ...
        for (let i = 0; i < batchCount; i++) {
          ...
          console.log(`  → Batch ${i + 1}/${batchCount}: Loading variants for products ${start + 1}-${end} (${batchIds.length} products)`);
          ...
          try {
            ...
            if (variantsData && variantsData.length > 0) {
              ...
              console.log(`  ✓ Batch ${i + 1}/${batchCount}: Loaded ${variantsData.length} variants`);
            } else {
              console.log(`  ✓ Batch ${i + 1}/${batchCount}: No variants found`);
            }
          } catch (error) {
        ...
        console.log(`✓ Successfully loaded ${allVariants.length} total product variants across all batches`);
      }
      ...
      <>
        ...
        <View style={[styles.container, styles.centerContent]}>
          ...
          <Text style={styles.emptyText}>
            ...
            Tutti gli articoli potrebbero essere esauriti o iPerfect! Now I understand the full picture:

**Issues identified:**

1. **Bad Request Error**: The app is trying to load variants for 3,678 products (1617 + 1999 + 62) in a SINGLE query, which creates a URL that's too long and causes a 400 Bad Request error.

2. **Only 1 list showing**: Because the variant loading fails, the code might be filtering out lists or products incorrectly.

**Solution:**

I need to restore batch loading for variants ONLY (not for products), but with a smarter approach that:
- Loads products in a single query (this works fine)
- Loads variants in smaller batches to avoid URL length limits
- Uses a reasonable batch size (e.g., 100 product IDs per request)
