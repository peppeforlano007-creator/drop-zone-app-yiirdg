  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [dropResult, bookingsResult] = await Promise.all([
        ...
        supabase
          .from('bookings')
          .select('id, product_id, original_price, final_price, discount_percentage, loyalty_discount, status, pickup_point_id, products!bookings_product_id_fkey(id, name, image_url)')
          .eq('drop_id', dropId)
          .eq('user_id', user.id)
          .eq('status', 'confirmed'),
      ]);
        ...
        {bookings.length === 0 ? (
          ...
          bookings.map((booking) => {
            ...
            const dropFinalDiscount = drop?.final_discount_percentage != null
              ...
              : Math.max(0, Number(booking.discount_percentage) - loyaltyPct);
