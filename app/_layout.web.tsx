...
function CustomSplashScreen({ onFinish }: { onFinish: () => void }) {
  ...
  useEffect(() => {
    ...
    Animated.parallel([
      Animated.spring(rdnScale, { toValue: 1, friction: 5, tension: 50, useNativeDriver: false }),
      Animated.timing(rdnOpacity, { toValue: 1, duration: 400, useNativeDriver: false }),
      Animated.timing(glowOpacity, { toValue: 0.6, duration: 600, useNativeDriver: false }),
    ]).start(() => {
      // Phase 2: STREET STOCK MARKET appare lettera per lettera (600-1400ms)
      Animated.stagger(120, [
        Animated.timing(streetOpacity, { toValue: 1, duration: 300, useNativeDriver: false }),
        Animated.timing(stockOpacity, { toValue: 1, duration: 300, useNativeDriver: false }),
        Animated.timing(marketOpacity, { toValue: 1, duration: 300, useNativeDriver: false }),
      ]).start(() => {
        // Phase 3: tagline appare (1400-1800ms)
        Animated.timing(taglineOpacity, { toValue: 1, duration: 400, useNativeDriver: false }).start(() => {
          // Phase 4: pausa poi fade out (2200-2700ms)
          setTimeout(() => {
            Animated.timing(containerOpacity, { toValue: 0, duration: 500, useNativeDriver: false }).start(() => {
              onFinish();
            });
          }, 600);
        });
